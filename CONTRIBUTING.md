...
¡Gracias por tu interés en contribuir a **Plantilla Portafolio Básico**! Esta guía describe el flujo de trabajo, los estándares de código y el proceso de Pull Requests.
...
Sigue las instrucciones del [archivo README](README.md#instalación). No hay dependencias que instalar: para trabajar en local basta con servir el sitio de forma estática (p. ej. `python3 -m http.server 8000`) o abrir `index.html`.
...
- Indentación y estilo definidos por [`.editorconfig`](.editorconfig); formatea la documentación con Prettier (`.github/scripts/format-markdown.sh`).
- Líneas de máximo 80 caracteres.
- Codificación UTF-8, finales de línea LF.

### Nombrado

- Sigue las convenciones idiomáticas de JavaScript (`camelCase` para variables y funciones, `PascalCase` para clases).
...
- No hay suite automatizada: verifica manualmente en el navegador antes de abrir el PR.
- Revisa la página en tema claro y oscuro y comprueba que los repos se cargan sin errores.
