...
Este documento describe cómo reportar vulnerabilidades en **Plantilla Portafolio Básico** y qué prácticas de seguridad seguimos en el repositorio.
...
| Releases / tags         | Solo la última versión publicada |
...
- El sitio no tiene dependencias de ejecución: es HTML/CSS/JS estático sin gestor de paquetes, por lo que no hay auditoría de dependencias que ejecutar.
- La única integración externa es la **API pública de GitHub**, que se consume sin token ni credenciales.
- **Dependabot** (`.github/dependabot.yml`) vigila las versiones de las GitHub Actions usadas en los workflows — sus PRs se revisan antes del merge.
